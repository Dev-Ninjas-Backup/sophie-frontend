...
const Register: React.FC = () => {
  ...
  const pollRegistration = async (
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        const checkRes = await axios.get(
          `${BASE_URL}/register/${registrationId}`,
        ...
        if (registrationData?.isActive == true) {
          ...
          // console.log(i)
          await axios.get(
            `${BASE_URL}/register/send-mail/${registrationData.id}`,
          );
    ...
    throw new Error(
      "Payment confirmation timed out. Please check your payment.",
    );
  };
  ...
  const handleSubmit = async (token: string) => {
    ...
    try {
      ...
      if (formData.teudatZehut.length !== 9) {
        toast.warn("Teudat Zehut must be exactly 9 digits");
        return;
      }
      ...
      toast.success(
        "Membership activated and Membership ID sent to your email.",
      );
  ...
  return (
    <div className="min-h-[50vh] flex flex-col font-abc-light  text-white items-center justify-center p-4">
      <h1 className="text-5xl text-center mb-12 md:text-6xl lg:text-7xl xl:text-8xl font-abc-heavy-3  bold-stroke-3 leading-10 md:leading-12 lg:leading-15 xl:leading-19 tracking-[-2px]  xl:tracking-[-5px] ">
        BECOME A MEMBER
      </h1>
          ...
          <div className="flex flex-col mb-6">
            ...
            <div className="flex gap-4 flex-1 border-b pb-4 border-b-gray-700">
              ...
              <div className="relative w-full">
                ...
                <p
                  className={`text-base lg:text-lg  my-2 ${currentStep === 2 ? "text-white" : "text-gray-500"}`}
                >
                  Pay & register
                </p>
              </div>
